...
`validation.config.PRICING` to measured units, embedding and rerank included — without that, the
...
Agent `us.anthropic.claude-opus-4-8`, region `us-east-1`, one replay per configuration. The large
differences — **−52.5%**, **−80.0%** and the accuracy holding at 95.8% while cost drops 79% — support a
decision; nothing under ~20% does, because that is inside the noise this harness has already measured on
replayed runs. Full breakdown (per-token cost attribution, per-turn curves, plateau and slope) is
regenerated by the harness under `validation/`.
...
arithmetic is right, which is deliberate. See `validation/accuracy.py`.
