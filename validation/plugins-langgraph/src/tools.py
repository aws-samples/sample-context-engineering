...
**Adapted from the Strands harness, and only at the decorator.** This file is
``validation/community-plugin-A-B-D/src/tools.py`` with three changes, all of them about how a
tool is *declared*, none about what it *returns*:

1. ``from strands import tool`` became ``from langchain_core.tools import tool``.
2. :func:`_converse_spec` was added, because the filler budget used to be measured off Strands'
   ``built.tool_spec`` attribute. It rebuilds the same Converse shape from the LangChain tool.
3. ``_generate_filler_tools`` calls that helper instead of reading the attribute.

Every function body is byte-identical, so every payload the model sees is byte-identical --
which is the precondition for ``ground_truth.py`` and ``accuracy.py`` being reusable unchanged.
A deterministic scorer matching ``R$ 14.454,00`` in an answer cannot tell the two harnesses
...
_FILLER_TOOL_COUNT = os.environ.get("VALIDATION_FILLER_TOOL_COUNT")
"""Pin the filler suite to an exact tool count instead of sizing it by token budget.

**Why this exists, and it is a real divergence rather than a knob for its own sake.** The Strands
harness and this one calibrate the same way -- generate filler until :data:`TARGET_SCHEMA_TOKENS`
is met -- but LangChain's pydantic-generated JSON schema is a few percent fatter per tool than
Strands' own ``tool_spec``, so the same budget fills with FEWER tools: measured, 68 here against
the 74 every recorded Strands run registered (``plugin_counters.registered_tools`` = 93 on the
baseline arm = 18 core + 1 web + 74 filler).

Budget-matching is the default because the budget is what config.py documents as the calibration,
and schema mass -- not tool count -- is the quantity the disclosure arm removes. But tool count is
what the lexical index has to discriminate between, so a count-matched comparison is a legitimate
second reading: set ``VALIDATION_FILLER_TOOL_COUNT=74`` for it. Both numbers are recorded in every
run's meta, so a table can never be read without knowing which one produced it.
"""


def _converse_spec(built: Any) -> dict[str, Any]:
    """Return ``built``'s schema in the shape Bedrock Converse receives it.

    **This is the one adaptation the filler budget needed.** The Strands harness sized its suite
    off ``built.tool_spec`` -- a Strands attribute that does not exist on a LangChain ``BaseTool``.
    Rebuilding the same three-key Converse shape here, rather than measuring some LangChain-native
    serialization, is what keeps :data:`TARGET_SCHEMA_TOKENS` meaning the same thing in both
    harnesses: the budget is still counted against the bytes that actually ride on every model
    call, so the schema mass the disclosure arm removes is the same mass.

    ``langchain_aws`` assembles ``{"toolSpec": {"name", "description", "inputSchema": {"json": …}}}``
    from exactly these three fields, so this is the wire payload and not an approximation of it.

    Args:
        built: A LangChain tool, as returned by :func:`_make_filler`.

    Returns:
        The Converse tool spec: ``name``, ``description`` and ``inputSchema.json``.
    """
    openai_form = convert_to_openai_tool(built)["function"]
    return {
        "name": openai_form["name"],
        "description": openai_form.get("description", ""),
        "inputSchema": {"json": openai_form.get("parameters", {})},
    }
...
def _generate_filler_tools(target_schema_tokens: int) -> list[Any]:
    ...
    ``VALIDATION_FILLER_TOOL_COUNT`` replaces the budget with an exact count; see
    :data:`_FILLER_TOOL_COUNT` for why that override is here.
    """
    tools: list[Any] = []
    accumulated = 0
    ceiling = int(_FILLER_TOOL_COUNT) if _FILLER_TOOL_COUNT else None

    for suffix in ["", *_FILLER_DOMAINS]:
        for name, subject, verb in _FILLER_SPECS:
            if ceiling is None and accumulated >= target_schema_tokens:
                return tools
            if ceiling is not None and len(tools) >= ceiling:
                return tools
            ...
            accumulated += len(json.dumps(_converse_spec(built), ensure_ascii=False)) // 4
            tools.append(built)

    return tools


def filler_schema_tokens() -> int:
    """Return the estimated Converse schema tokens the generated filler suite costs.

    Recorded in each run's meta beside the tool count, so the two harnesses' calibrations are
    comparable from the JSON alone rather than by rebuilding both suites.
    """
    return sum(len(json.dumps(_converse_spec(built), ensure_ascii=False)) for built in FILLER_TOOLS) // 4
