...
This harness is the **LangGraph sibling** of ``validation/community-plugin-A-B-D``. It measures the
same three strategies against the same script, the same mocked tools and the same deterministic
ground truth, but the agent is a LangChain v1 ``create_agent`` graph and the strategies are
``AgentMiddleware`` implementations rather than Strands plugins:

======================  ==============================================  ==============================================
Strategy                Strands (community-plugin-A-B-D)                LangGraph (here)
======================  ==============================================  ==============================================
Relevance filtering     ``strands_relevance_filter.RelevanceFilter``     ``langgraph_relevance_filter.RelevanceFilterMiddleware``
Progressive disclosure  ``strands_progressive_tool_disclosure…``         ``langgraph_progressive_tool_disclosure…``
Context graph           ``strands_context_graph.ContextGraph``           ``langgraph_context_graph.ContextGraphMiddleware``
======================  ==============================================  ==============================================

**This file is a verbatim copy of the Strands harness's ``config.py`` apart from this paragraph.**
That is deliberate and it is the whole basis of the comparison: :data:`MODEL_PRICING`,
:data:`THRESHOLDS`, :data:`GRAPH_TUNING`, :data:`TARGET_SCHEMA_TOKENS` and :func:`estimate_tokens`
are byte-identical to the ones the published Strands numbers were produced with, so a difference
between the two harnesses' tables cannot come from a re-tuned knob or a re-priced rate. The one
Strands-only setting that survives here without meaning is :data:`SESSION_MANAGER`: LangGraph's
own checkpointer takes that role, and the LangGraph runner ignores the value.
