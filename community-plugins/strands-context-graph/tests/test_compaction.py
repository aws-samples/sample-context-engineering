...
``InjectionContext`` is the real dataclass with ``state`` and ``agent`` left as ``None``: the render reads ``messages``
and nothing else, and a double would only hide that.
"""
...
from strands_context_graph.compaction import _FRAGMENT_INDENT, render_final_block
...
def test_nothing_is_mutated():
    ...
    messages_before = copy.deepcopy(injection_context.messages)
