...
:func:`is_turn_boundary` delegates to ``_is_user_turn`` from ``strands.injection._message_injection``, the
``"userTurn"`` policy of Requirement 3.1: a ``user`` message carrying no ``toolResult`` block. Reused rather than
reimplemented so the graph's notion of a turn is the SDK's.
...
**Private-API dependency** (Requirement 17.4). ``_is_user_turn`` is imported statically and is a *hard* dependency: a
rename fails at import rather than degrading. Deliberate, for the reason the delegation exists — a local copy of the
boundary rule would drift from the SDK's, and a drifted boundary silently mis-splits every turn, so every Card describes
the wrong span while nothing raises. A public turn-boundary predicate would remove the coupling; see the README's
private-API table for the whole set.
"""
