...
| Raw tool return | the plugin's own ``ReferenceStore`` | ``reference`` | yes, the key |
...
class _GraphState:
    ...
    The standalone graph is ephemeral: the state reaches neither ``agent.messages``, nor ``agent.state``, nor message
    metadata, nor a store of its own, nor disk. It lives only in the weakly keyed map below, so it is discarded with its
    agent (Requirements 14.4, 14.9, 1.5).
...
"""Per-agent state map, keyed weakly so the state is dropped with its agent, and independent per agent (Req. 14.1)."""
