...
This module defines the scoring contract used by the ``RelevanceFilter`` plugin
(:class:`~strands_relevance_filter.plugin.RelevanceFilter`) through
:class:`~strands_relevance_filter.preview.RelevancePreview`. The contract is intentionally narrow:
given a query and a list of chunk texts, return one score per chunk. Anything that prevents that
(transport failure, timeout, malformed response) surfaces as :class:`RerankerError`, so the caller has
a single exception type to catch when leaving the tool result unchanged.
...
``BedrockReranker`` is the default implementation and the plugin's only remote dependency; it is built
lazily, so an install that supplies its own ``Reranker`` never constructs an AWS client.
...
class RerankerError(Exception):
    ...
    allowed to raise from :meth:`Reranker.score`. The plugin treats it as a
...
class Reranker(Protocol):
    ...
    Implement this protocol to plug a custom scorer into the plugin. The
...
class BedrockReranker:
    ...
    same pattern as :class:`~strands_relevance_filter.store.S3Store`: an
    optional ``boto_session``, ``region_name`` honored only when no session is
    given, and ``user_agent_extra="strands-agents"`` merged into the client
    config.

    Timeouts default to 10 seconds for both connect and read. A rerank call
    that outlives them fails fast so the plugin can leave the tool result
    ...
    Example:
        ```python
        from strands_relevance_filter import BedrockReranker
