...
from context_core.graph import store as store_module
...
def _register_host_symbols(monkeypatch: pytest.MonkeyPatch, symbols: dict[str, object]) -> None:
    """Register fake host symbols into ``HOST_SYMBOLS`` for the duration of a test.

    Standalone, ``HOST_SYMBOLS`` is empty and every optional bridge is absent. To exercise the bridge without a
    framework installed, this installs a throwaway module carrying ``symbols`` and points the corresponding
    ``HOST_SYMBOLS`` keys at it — so the real ``_optional_symbol`` / ``import_module`` path resolves them, exactly as a
    framework binding's registration would. Keys left out stay absent, taking the same local fallback an empty registry
    takes.

    Args:
        monkeypatch: The test's monkeypatch fixture.
        symbols: A mapping of ``HOST_SYMBOLS`` key -> the object that key should resolve to.
    """
    module_name = "context_core_test_host_symbols"
    module = types.ModuleType(module_name)
    registry: dict[str, tuple[str, str]] = {}
    for key, value in symbols.items():
        attr = f"_sym_{key}"
        setattr(module, attr, value)
        registry[key] = (module_name, attr)
    monkeypatch.setitem(sys.modules, module_name, module)
    monkeypatch.setattr(store_module, "HOST_SYMBOLS", registry)


class _Stash:
    """A stand-in for a host manager's Stash: one async read, nothing else."""
...
class _FakeContextManager:
    """A stand-in for the host's context manager, carrying only the attribute the bridge reads."""
...
def _with_manager(monkeypatch: pytest.MonkeyPatch, manager: object) -> _Agent:
    """An agent whose registry carries ``manager``, with the bridge's anchor type registered as the fake."""
    _register_host_symbols(monkeypatch, {"context_manager": _FakeContextManager})
...
async def test_a_renamed_context_manager_degrades_to_no_bridge(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(store_module, "HOST_SYMBOLS", {})
...
def test_a_targeted_read_is_bounded_by_the_shared_ceiling(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _register_host_symbols(monkeypatch, {"search_content": _search})
