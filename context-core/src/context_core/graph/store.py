...
**Where the host symbols come from.** The core names no framework module of its own. The three optional bridges above
are looked up through :func:`_optional_symbol` against ``(module, name)`` pairs read from :data:`HOST_SYMBOLS`, a
module-level registry that is **empty by default**. A framework binding registers its own module paths into
``HOST_SYMBOLS`` at its boundary, under the documented keys ``"extract_text"``, ``"search_content"`` and
``"context_manager"``. An empty registry is the standalone configuration: every bridge is absent and every call site
takes the identical local fallback it would take were the named symbol missing from the host.
"""
...
HOST_SYMBOLS: dict[str, tuple[str, str]] = {}
"""Caller-overridable registry of the optional host symbols this module bridges to, empty by default.

The core must not name a framework module, so the three optional bridges are looked up through this registry instead of
through hardcoded module paths. A framework binding registers its own ``(module, name)`` tuples here at its boundary,
under these documented keys:

- ``"extract_text"``: the host's text-recovery helper, used by :func:`_extract_text`.
- ``"search_content"``: the host's line/pattern search helper, used by :func:`_search_helper`.
- ``"context_manager"``: the host's manager anchor type, used by :func:`_stash_of`.

An **empty** registry — the default, standalone configuration — means every one of these keys is absent, and each call
site takes exactly the same fallback path the source takes when the corresponding SDK symbol does not resolve: bare
strings only for text recovery, whole reads only (targeted reads degrade to prose), and no Stash interop. Each fallback
is complete on its own; nothing here raises when a key is missing."""
...
def _stash_of(agent: object) -> object | None:
    ...
    Returns:
        The Stash, or ``None``.
    """
    manager_type = _host_symbol("context_manager")
...
def _extract_text(block: object) -> str | None:
    ...
    """
    helper = _host_symbol("extract_text")
    if callable(helper):
        try:
            text = helper(block)
        except Exception:
...
def _search_helper() -> Callable[..., str] | None:
    ...
    """
    helper = _host_symbol("search_content")
...
def _host_symbol(key: str) -> Any | None:
    """Resolve one optional host symbol registered under ``key`` in :data:`HOST_SYMBOLS`, or ``None``.

    The core never names a framework module of its own, so the ``(module, name)`` pair is read from the caller-supplied
    registry. A ``key`` the registry does not carry — the standalone default, where the registry is empty — answers
    ``None`` without any import attempt, which is exactly the ``None`` :func:`_optional_symbol` returns for a symbol
    that does not resolve. Every call site therefore takes the same fallback path whether the registry is empty or the
    named symbol is simply absent from the host.

    Args:
        key: One of the documented :data:`HOST_SYMBOLS` keys.

    Returns:
        The resolved symbol, or ``None``.
    """
    location = HOST_SYMBOLS.get(key)
    if location is None:
        return None
    return _optional_symbol(*location)
