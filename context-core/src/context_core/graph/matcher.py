...
The embedding client is a ``bedrock-runtime`` client built on first need, following the usual Bedrock path (a
``boto3.Session``, botocore timeouts, ``user_agent_extra="context-core"``). ``boto3`` is imported inside that step and
...
class EmbeddingSimilarityMatcher:
    ...
    Example:
        ```python
        from context_core.graph.matcher import EmbeddingSimilarityMatcher
