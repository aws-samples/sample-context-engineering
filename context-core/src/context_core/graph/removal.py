...
**Tool-pair rule** (Requirement 17.4). ``_get_tool_use_ids`` and ``is_pinned`` are reimplemented locally, verbatim in
behaviour, alongside the public ``is_pinned``, so the tool-pair rule lives in one place. The failure mode of a drifted
tool-pair index is a ``toolUse`` sent without its ``toolResult`` — a protocol error the provider rejects, discovered at
runtime and attributed to the model. Keeping the rule local and explicit keeps the retained ``toolUse`` and
``toolResult`` sets equal.
"""
...
def _get_tool_use_ids(message: NeutralMessage) -> set[str]:
    """Extract toolUseIds from toolUse or toolResult blocks in a message.

    Local reimplementation of the SDK primitive of the same name, verbatim in behaviour.
    """
    ids: set[str] = set()
...
def _has_pinned_flag(message: NeutralMessage) -> bool:
...
def is_pinned(messages: Sequence[NeutralMessage], index: int) -> bool:
...
def remove_messages(messages: Sequence[NeutralMessage], drop_ids: frozenset[str]) -> list[NeutralMessage]:
...
def apply_removal(
    ...
    choice: TurnChoice,
    current_turn_ids: frozenset[str],
) -> tuple[list[NeutralMessage], frozenset[str]]:
...
def _first_user_index(messages: Sequence[NeutralMessage]) -> int:
...
def _protected_indices(messages: Sequence[NeutralMessage], first_user_index: int) -> set[int]:
...
def _provisional_keep(messages: Sequence[NeutralMessage], drop_ids: frozenset[str], protected: set[int]) -> list[bool]:
...
def _tool_pair_groups(messages: Sequence[NeutralMessage]) -> tuple[list[set[str]], dict[str, list[int]]]:
