...
the message history. The caller holding the messages passes ``costs``; without it the count of durable identities stands
...
def record_reuse(state: _GraphState, title: str, cycle: int, *, reuse_ttl_cycles: int) -> None:
    ...
    Args:
        ...
        cycle: Current cycle counter, supplied by the host.
...
def full_pass_choice() -> TurnChoice:
    ...
    Returns:
        ...
        object identity, so the assembled context is identical field by field to the one produced without the
        projection.
    """
