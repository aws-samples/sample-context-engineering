...
Everything here is a scan over messages: no model call, no embedding, no I/O (Requirement 3.2).

:func:`is_turn_boundary` delegates to ``_is_user_turn``, a local reimplementation of the host's turn primitive, the
``"userTurn"`` policy of Requirement 3.1: a ``user`` message carrying no ``toolResult`` block. Reused rather than
reimplemented so the graph's notion of a turn is the host's.
...
within the same turn (Requirements 6.4, 6.5).
...
read off the preview text, while the raw return stays in the plugin's own ``ReferenceStore``. Hook order against an
offloader is therefore irrelevant. With nothing offloading, no placeholder replaces a tool return, the scan finds no
reference, and there is no artifact Card (Requirement 15.5).

:func:`rebuild` composes the same helpers over a whole conversation, so a lost graph costs a scan and never a value. It
runs on the writing half only.

**Turn-boundary primitive** (Requirement 17.4). ``_is_user_turn`` is a local reimplementation of the host's boundary
rule, verbatim in behaviour. A local copy risks drifting from the host's rule, and a drifted boundary silently
mis-splits every turn, so every Card describes the wrong span while nothing raises; the copy is kept byte-faithful for
that reason. A public turn-boundary predicate on the host would remove the coupling.
"""
...
from .state import Card, Link, LinkKind, ToolPair, _GraphState


def _is_user_turn(messages: Sequence[NeutralMessage]) -> bool:
    """Whether the latest message is a fresh user ask: a ``user`` message carrying no tool result.

    Local reimplementation of the host's turn primitive of the same name, verbatim in behaviour, so the core
    carries no framework import.
    """
    if not messages:
        return False
    last = messages[-1]
...
``scoring`` and is applied where the Note inherits, so carrying it here too would multiply it in twice. ``Link.weight``
carries a measurement or nothing, and similarity is the only kind with one to carry."""

_STORED_REFERENCES = "[Stored references:]"
"""Header an offloader writes above its reference listing. Matched as a substring, not a whole line: a listing that
...
An offloader's non-text placeholders name one: ``| ref: mem_1_tu-3_0``. The ``ContextManager``'s Stash names one the
same way and several as ``[refs: tu-3_0, tu-3_1]``, so a comma-separated run is accepted and the capture is split on
commas. Each reference still stops at whitespace, ``|`` and ``]`` — the characters that close the field and that a
reference never contains — so a sentence that merely mentions a reference does not absorb the words after it."""

_LISTED_REFERENCE = re.compile(r"^[ \t]+(\S+)[ \t]*\(")
"""A reference on its own line under the listing header: indentation, the reference, its description. Anchored on the
...
"""A size stated in bytes, thousands separators included. ``chars`` is not matched: a text block's length is stated in
characters, which are not bytes in any encoding. A size that would have to be guessed is left absent."""
...
"""Media type of the two descriptors a listing writes as a bare word rather than as a media type."""


def is_turn_boundary(message: NeutralMessage) -> bool:
...
def turn_ranges(messages: Sequence[NeutralMessage]) -> tuple[tuple[int, int], ...]:
    ...
    it is :func:`closed_turn_ranges`' question, not this one's.
...
def closed_turn_ranges(messages: Sequence[NeutralMessage]) -> tuple[tuple[int, int], ...]:
...
def is_evidence(message: NeutralMessage) -> bool:
...
def partition_turn(turn_messages: Sequence[NeutralMessage]) -> tuple[tuple[str, ...], tuple[str, ...]]:
    ...
    tuples, decided by :func:`is_evidence` alone. A message without one is a message without a Card and appears in
    neither (Requirement 3.5).
...
def tool_pairs_of(turn_messages: Sequence[NeutralMessage]) -> tuple[ToolPair, ...]:
...
def _blocks(message: NeutralMessage) -> Iterator[Mapping[str, Any]]:
...
def _carries_text(message: NeutralMessage) -> bool:
...
def derive_card(
    ...
    *,
    ...
    part in no field of the result, and projects whole (Requirement 3.5). Cards are grouped by turn and by nothing else
    — no subject classification, which would need a model (Requirement 3.11). Tags here rank rarity as if this Card were
    the graph's only one; :func:`retag` recounts over the whole graph right after registration.
...
def register_card(
    ...
    *,
    ...
    - **artifact**, one per reference cited in a preview, targeting the reference. The reference is also an artifact
      Card's Title, so the edge resolves whether or not that Card exists yet, making hook order against an offloader
      irrelevant.
    ...
    # Sorted, not in frozenset order: string hashing is seeded per process, and link order has to be the same whether
    # the graph was built incrementally or rebuilt by scan.
...
def rebuild(
    ...
    *,
    ...
    Every Card, link and turn ordinal is derivable from the host's message history, so a state lost to a restart is
    rebuilt from
    ...
    Returns:
        ...
        same conversation, and whose ``turn`` is the number of closed turns.
    """
    state = _GraphState()
    rebuild_into(
        state,
...
def _identities_of(turn_messages: Sequence[NeutralMessage]) -> tuple[str, ...]:
...
def retag(state: _GraphState, messages: Sequence[NeutralMessage], *, tags_per_card: int, rarity_weight: float) -> None:
...
def derive_artifact_cards(
    ...
    *,
    ...
    A result carrying no reference yields no Card, which is the whole nothing-offloaded path (Requirement 15.5): with no
    placeholder replacing the tool return, the scan finds no reference, an empty tuple comes back, and the subject Cards
    are untouched. No branch asks whether an offloader is present.

    The reference is read out of the preview text, so hook order against an offloader costs at most a turn of latency:
    the same scan reaches the same references later from the host's message history.
...
def register_artifact_cards(
    ...
    *,
    ...
    an artifact is not a turn, and ``similar`` would propagate Note into a Card that is kept out of full content by
    automatic choice.
    ...
    for card in cards:
        state.cards[card.title] = card
        state.links.setdefault(card.title, [])
        # Sorted for the same reason as in ``register_card``: seeded string hashing would otherwise make the rebuilt
        # link order differ from the incremental one.
...
def derive_and_register_artifacts(
    ...
    *,
    ...
    naming no reference is not a failure and logs nothing: it is the ordinary shape of a result nothing replaced
    (Requirement 15.5).
...
def _artifact_card(
    ...
    *,
    ...
    reference store and not messages, so it owns no durable identity and no resolution of its parts can drop a message.
    ``references`` is empty because the address lives in ``reference`` alone.
...
def _artifact_metadata(texts: Sequence[str], references: Collection[str]) -> dict[str, tuple[str | None, int | None]]:
    ...
    Two sources, because a preview writes two. The listing under ``[Stored references:]`` states a descriptor per
...
def _descriptor_facts(descriptor: str) -> tuple[str | None, int | None]:
    ...
    The head is the field before the first comma. ``text`` and ``json`` are the two bare words a listing writes;
...
def _byte_count(digits: str) -> int | None:
    ...
    Args:
        digits: The digits as the preview wrote them, separators included.
...
def _title_of(turn_messages: Sequence[NeutralMessage]) -> str:
...
def _link(state: _GraphState, source: str, kind: LinkKind, target: str, weight: float) -> None:
    ...
    edge = Link(kind=kind, target=target, weight=weight)
...
def _cached_similarity(state: _GraphState) -> Callable[[Card, Card], float | None]:
    ...
    def measure(left: Card, right: Card) -> float | None:
        ...
        return _cosine_similarity(left_entry[1], right_entry[1])

    return measure


def _cosine_similarity(left: Sequence[float], right: Sequence[float]) -> float:
    """Measure the cosine similarity of two vectors of equal length.

    Local arithmetic rather than a dependency: the graph compares a few dozen vectors per turn, and pulling a numeric
    stack in for one dot product would make a plugin's dependency everyone's.
    ...
    Returns:
        The similarity, in ``[-1.0, 1.0]``. ``0.0`` when the lengths differ or either vector has no magnitude, the two
        cases where the measure is undefined rather than small.
    """
...
def _texts_by_id(messages: Sequence[NeutralMessage]) -> dict[str, tuple[str, ...]]:
...
def _texts_of(message: NeutralMessage) -> tuple[str, ...]:
    ...
    artifact link and hook order against an offloader would start to matter.
...
def _references_of(texts: Sequence[str]) -> tuple[str, ...]:
    ...
    Two shapes, because the two hosts write two. A non-text block becomes a placeholder naming its reference inline —
    ``[image: png, 900 bytes | ref: mem_1_tu-3_0]`` — the Stash writes a standalone ``[ref: x]`` or ``[refs: x, y]``,
    and every stored block is listed under a ``[Stored references:]`` header, one indented line each. Reading the
    reference off the preview text makes the graph independent of hook order against an offloader: by the time a
    ``toolResult`` is a message, its preview names every reference it produced.
