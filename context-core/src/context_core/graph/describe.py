...
- ``title_for`` returns a literal prefix: ``user_text.startswith(title_for(user_text))``. There is no ellipse, which
...
- ``compose_description`` assembles a header out of addresses and appends the selected lines verbatim, so the
  Description minus its trailing omission line is a literal prefix of the complete Description.
...
the same scan, and they add one promise: no embedding call either (Requirement 6.5).
"""
...
_CHARS_PER_TOKEN = 4
"""Characters per token, a coarse provider-agnostic estimate: the budget exists to bound growth, not to be exact."""
...
separators. Copy-pasted spreadsheet output carries the unicode ones, and a pattern knowing only ``.`` and ``,`` skips
those lines."""
...
_THOUSANDS_GROUP = 3
"""Digits in a group of thousands. Three trailing digits after a lone separator reads as ``1.200``, not as a
fraction."""
...
"""How many cell separators make a line a table row. Two: a single ``|`` is as likely to be prose or a code fragment as
a row."""
...
def numeric_lines(texts: Sequence[str]) -> tuple[str, ...]:
    ...
    (Requirement 4.8). Exact duplicates are dropped.
...
def compose_description(card: Card, description_tokens: int) -> str:
    ...
    No model call, for any kind of Card (Requirement 4.7). The Description is a header of addresses followed by the
    Card's numeric lines copied verbatim; the budget decides how many lines get in, never how they are worded.

    Three shapes:

    - **Subject** (Requirement 4.2): the subject name, the tools used with their call counts, the cited references, then
      the numeric lines.
    - **Textual artifact**: the reference, the tool that produced the content, the turn ordinal, then
      the numeric lines.
    - **Non-textual artifact**: only the file name, the ``content_type``, the size, the tool, the turn
    ...
    omitted)`` records the rest. Only when the header alone overruns does the cut fall back to sentence boundary, then
    word boundary, then character count, and no ellipse, which would cost the prefix property.
...
def tag_candidates(card: Card, texts: Sequence[str] = ()) -> tuple[tuple[str, ...], dict[str, int]]:
    ...
    Three sources and nothing else (Requirement 6.2): the name of a ``toolUse`` block, an artifact reference, and a
...
def select_tags(
    ...
    6.3): a tool name and an artifact reference are what the turn did, not what it talked about. A Card with five tool
    ...
    Repetition alone promotes the domain vocabulary, which distinguishes no two turns; rarity alone
    promotes the typo that occurred once. The rarity term is inverse frequency counted over the graph's Cards, so what
    defines a Card depends on what it is compared against, hence the re-tag when the graph gains one.

    Ties break by order of first appearance in ``candidates``, so two runs over the same graph and configuration produce
    the same tags Card by Card (Requirement 6.6). No model call and no embedding call (Requirement 6.5): counting and
    arithmetic only.
    ...
    taken = set(tags)
    for _position, (token, _count) in ranked:
...
def normalize(token: str) -> str:
    ...
    A monetary amount and a separator-bearing number collapse onto the same form (Requirement 6.4): ``R$ 1.200,00``,
...
def _subject_header(card: Card) -> str:
    """Assemble the address part of a subject Card's Description (Requirement 4.2).
...
def _artifact_header(card: Card, textual: bool) -> str:
    """Assemble the address part of an artifact Card's Description.
...
def _tool_counts(card: Card) -> tuple[tuple[str, int], ...]:
    ...
    between runs and break Requirement 4.8. A tool named without a pair is counted once, being a ``toolUse`` block whose
    result has not been paired, a call that happened with no way to tell whether it happened twice.
...
def _cut(text: str, max_chars: int) -> str:
    ...
    The last sentence boundary that fits, then the last word boundary, then the character ceiling when neither fits — a
    single token longer than the budget, where a boundary-based cut would return nothing. No ellipse: the prefix
...
def _estimate_tokens(text: str) -> int:
    ...
    The same heuristic the Description is cut by, so a caller measuring a Description against a budget measures it the
    way ``compose_description`` filled it.
...
def _rarity(document_frequency: int, total_cards: int) -> float:
    ...
    Strictly decreasing in ``document_frequency``: making a candidate rarer never lowers its position.
