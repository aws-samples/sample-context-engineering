"""Search helpers backing the retrieval tool: line-range spans and ReDoS-guarded pattern matching.

``line_range`` returns exactly the requested 1-indexed inclusive span with line numbers, matching the
line numbering the preview's gap markers report; ``pattern`` returns the matching lines together with
their surrounding context lines.

Pure and framework-agnostic: these helpers take and return plain strings, so a binding that exposes a
retrieval tool over the neutral message shape calls them directly.
"""
