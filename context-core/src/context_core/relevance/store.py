...
A binding writes the raw sub-blocks of a filtered tool result to a ``Store`` before rewriting the
...
Framework-agnostic: nothing here touches a message type. ``boto3`` / ``botocore`` are the only
third-party imports permitted, for the S3 backend.
"""
...
class Store(Protocol):
    """Backend for the raw tool-result content the relevance filter offloads.

    Each sub-block of a filtered tool result is stored individually with its content type preserved.
    The core ships ``InMemoryStore`` (the default), ``FileStore``, and ``S3Store``; implement this
...
class S3Store:
    ...
    Args:
        ...
        boto_client_config: Optional botocore client configuration. A caller-supplied
            ``user_agent_extra`` is extended with ``context-core`` rather than replaced.
        region_name: AWS region. Used only when ``boto_session`` is not given.

    Example:
        ```python
        from context_core.relevance.store import S3Store
    ...
    def __init__(
        ...
        if boto_client_config:
            ...
            new_user_agent = (
                f"{existing_user_agent} {_USER_AGENT_EXTRA}" if existing_user_agent else _USER_AGENT_EXTRA
            )
            client_config = boto_client_config.merge(BotocoreConfig(user_agent_extra=new_user_agent))
        else:
            client_config = BotocoreConfig(user_agent_extra=_USER_AGENT_EXTRA)
