"""The relevance-scoring seam: the ``Reranker`` protocol, its error type, and the Bedrock default.

This module defines the scoring contract used by :class:`~context_core.relevance.preview.RelevancePreview`.
The contract is intentionally narrow: given a query and a list of chunk texts, return one score per
chunk. Anything that prevents that (transport failure, timeout, malformed response) surfaces as
:class:`RerankerError`, so the caller has a single exception type to catch when leaving the tool result
unchanged.

``Reranker`` is stateless, scores the chunks of a single tool result, and needs a true ``[0.0, 1.0]``
because ``relevance_threshold`` reads it absolutely. It *must* raise instead of degrading, which is why
:meth:`BedrockReranker.score` refuses to clamp an out-of-range score: a wrong score here drops the
passage the question needed, and a wrong answer is worse than leaving the block as it was. A scorer
whose miss only costs tokens can clamp and degrade quietly; this one refuses to.

``BedrockReranker`` is the default implementation and this core's only remote dependency; it is built
lazily, so an install that supplies its own ``Reranker`` never constructs an AWS client. ``boto3`` /
``botocore`` are the only third-party imports permitted here — no agent framework.
...
_USER_AGENT_EXTRA = "context-core"
"""User-agent token merged into every client this module builds.

The Strands plugin this core was recreated from sent ``"strands-agents"`` here. The core is
framework-agnostic and may be driven by any binding, so it identifies itself instead; the *merge*
behaviour (a caller-supplied token is extended, never replaced) is unchanged.
"""
...
class RerankerError(Exception):
    ...
    allowed to raise from :meth:`Reranker.score`. The caller treats it as a
    signal to leave the tool result unchanged, never as a fatal error.
    """
...
class Reranker(Protocol):
    ...
    Implement this protocol to plug a custom scorer into the pipeline. The
    default is :class:`BedrockReranker`, built on the Bedrock ``rerank``
    operation.
...
class BedrockReranker:
    ...
    same pattern as :class:`~context_core.relevance.store.S3Store`: an
    optional ``boto_session``, ``region_name`` honored only when no session is
    given, and ``user_agent_extra="context-core"`` merged into the client
    config.

    Timeouts default to 10 seconds for both connect and read. A rerank call
    that outlives them fails fast so the caller can leave the tool result
    unchanged instead of stalling the agent loop. Pass ``boto_client_config``
    ...
    Example:
        ```python
        from context_core.relevance.reranker import BedrockReranker
        ...
        *,
        ...
        if boto_client_config:
            ...
            if existing_user_agent:
                user_agent_extra = f"{existing_user_agent} {_USER_AGENT_EXTRA}"
...
def _validated_score(item: Any) -> float:
    ...
    Raises:
        RerankerError: If the score is absent, not a finite real number, or
            ...
            describes, and the caller is better served by an unchanged tool
            result than by an invented value.
    """
