"""Search protocol over the full text of tool specifications — framework-agnostic.
...
``ToolIndex`` is the only text-scoring protocol in this family with a *build* phase, because tool
specifications are static and the work is worth doing once per registry fingerprint. It is also the
only one whose scores carry no absolute meaning — selection is ``top_k``, so no threshold reads them.
An ``EmbeddingToolIndex`` fits the same protocol: embed the need as ``"query"`` and the specification
texts as ``"document"``.

Neutral shape
-------------
A *tool specification* is a plain dict — ``{"name": str, "description": str, "inputSchema": {...}}`` —
so no agent framework is imported here. ``inputSchema`` may arrive wrapped as ``{"json": {...}}`` (the
Bedrock Converse shape) or unwrapped; both are tolerated.
"""
...
__all__ = ["LexicalToolIndex", "ToolIndex", "ToolMatch", "ToolSpec"]

ToolSpec: TypeAlias = dict[str, Any]
"""Neutral tool specification: ``name`` + ``description`` + ``inputSchema``, as a plain dict."""
