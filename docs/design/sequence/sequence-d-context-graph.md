...
|---|---|---|---|---|---|
| `MessageAddedEvent` | `plugin.py:648` (`agent.add_hook(self._on_message_added, MessageAddedEvent)`) | `_on_message_added` (`plugin.py:797`) | `agent.messages`; `closed_turn_ranges`, `is_turn_boundary` | `state.cards`, `state.links` (via `derive_and_register` / `_rebuild`) | Closes the turn *before* the boundary; also the ONLY place the rebuild scan may run (`plugin.py:798` docstring, `cards.py:rebuild:500`) |
| `AfterToolCallEvent` | `plugin.py:649` (`agent.add_hook(self._on_after_tool_call, AfterToolCallEvent)`) | `_on_after_tool_call` (`plugin.py:872`) | `event.result`, `event.tool_use['name']`, `agent.messages` | `state.cards`/`state.links` (artifact Cards) + the per-agent `InMemoryReferenceStore` (`record_references`, `plugin.py:915`) | Fast path only; the rebuild scan reaches the same references later, so hook order vs. an offloader is irrelevant (`plugin.py:873` docstring) |
| `BeforeInvocationEvent` | `plugin.py:647` (`agent.add_hook(self._on_before_invocation, BeforeInvocationEvent)`) | `_on_before_invocation` (`plugin.py:924`) | `state`, `event.messages`, `agent.event_loop_metrics.cycle_count` | `state.reuse` (expire), `state.choice`, `state.turn` (`+= 1`), `state.retrieval_cycles` (`= 0`), and `state.vectors` via `_cache_description_vectors` | On the critical path; the one embedding round of the turn happens here (`plugin.py:976`) |
...
Tools registered (auto-discovered as the three `@tool` members, `plugin.py:1070`):

| Tool | Defined | Decorator | Delegates to |
|---|---|---|---|
| `expand_card` | `plugin.py:1074` | `@tool(context=True)` (`plugin.py:1073`) | `tools.expand_card` (`tools.py:94`) |
| `expand_artifact` | `plugin.py:1102` | `@tool(context=True)` (`plugin.py:1101`) | `tools.expand_artifact` (`tools.py:192`) |
| `find_context` | `plugin.py:1145` | `@tool(context=True)` (`plugin.py:1144`) | `tools.find_context` (`tools.py:329`) |
...
sequenceDiagram
    ...
    participant P as ContextGraph._on_message_added<br/>(plugin.py:797)
    participant C as cards.py
    participant D as describe.py
    participant BI as _on_before_invocation<br/>(plugin.py:924)
    participant CV as _cache_description_vectors<br/>(plugin.py:998)
    ...
    BI->>S: compute_notes → matcher.score (one embedding round) (plugin.py:976)
    BI->>CV: _cache_description_vectors(state, matcher) (plugin.py:985)
    CV->>CV: matcher.vectors(descriptions) — free, cached under document purpose
    CV->>C: link_newly_measurable(state, newly_measurable, link_threshold) (plugin.py:1046 → cards.py:1020)
...
sequenceDiagram
    ...
    participant BI as _on_before_invocation<br/>(plugin.py:924)
    participant CC as _compute_choice<br/>(plugin.py:951)
    participant S as scoring.py
...
**The neighbour line does not fix this, and is not claimed to.** It makes *one* retrieval call worth more — a single `find_context` now returns up to 5 candidates plus up to 15 neighbour titles instead of 5 candidates — but it still only pays off on a turn where the model calls a retrieval tool at all. Unmeasured: the `similar` edge had no reader in any published run (`config.py:515`).
...
The `@tool` decorator derives the tool description from the docstring and the input schema from the typed signature. Signatures: `expand_card(self, titles: list[str], tool_context)` (`plugin.py:1074`); `expand_artifact(self, reference: str, tool_context, line_range: dict[str, int] | None = None, pattern: str | None = None)` (`plugin.py:1102`); `find_context(self, need: str, tool_context, tag: str | None = None)` (`plugin.py:1145`). `tool_context` is framework-injected, not part of the model-facing schema.

`expand_card` docstring (`plugin.py:1075`–`1091`):
...
`expand_artifact` docstring (`plugin.py:1102`, body at lines 1109–1129):
...
There is **no** fourth retrieval tool. Only these three `@tool` members exist (`plugin.py:1070` comment: "the only ``@tool`` members of the class").
...
Then per candidate: `- title: {title}` (`tools.py:518`), an optional `  tags: {', '.join(card.tags)}` (`tools.py:520`), the Description lines indented by two spaces (`tools.py:523`), and — new — the neighbour line when `neighbors_per_candidate > 0` and the Card has `similar` edges (`tools.py:527`):
...
| `matcher` | `None` → default `EmbeddingSimilarityMatcher` resolved lazily (`plugin.py:_matcher_for:1048`) | **Yes** — `None` or an object with a callable `score` (`_validate_matcher:415`) |
...
The harness used to hold two tuning sets and pick between them at build time with `GRAPH_WITH_RELEVANCE if config.relevance else GRAPH_ALONE` — i.e. **the graph inspected whether the relevance filter was installed in order to decide how much to fold.** Both names are gone from `config.py` and `runner.py`; `build_plugins` (`runner.py:325`) now reads one set unconditionally, `tuning = GRAPH_TUNING` (`runner.py:391`), and records `config.extra["_graph_tuning"] = "unified"` (`runner.py:392`).

The ruling is a design argument, not a new measurement, and it is recorded in `GraphTuning`'s docstring (`config.py:450`):
...
So the filter makes the graph's input *smaller*, not *different in kind*, and a knob that decides how aggressively to fold a history has no business reading whether some other plugin trimmed that history first. The measurement that originally justified the split is also confounded — it ran `preview_tokens` at 800 against payloads ten to thirty times that size, so the Cards were starved by the **first** cut in the chain and a larger Description budget had nothing left to preserve. `preview_tokens` is 2,000 now (`config.py:319`), so the condition that produced the result no longer holds.

No coupling to the filter remains. The last one was not a tuning value: `include_artifact_tool=not config.relevance`, kept because two retrieval tools over two stores that do not know each other is a model-facing ambiguity rather than a folding decision. It is gone — the filter registers no retrieval tool since `include_retrieval_tool` defaults to `False`, so there is no second store to disambiguate from and the drop only cost the combined arm its artifact-recovery path. The harness now passes `include_artifact_tool=True` in every arm (`runner.py:413`).

### 10.2 The unified values

`GRAPH_TUNING` (`config.py:518`), every field env-overridable so a sweep needs no commit:

| Knob | Value | Env override | Note |
|---|---|---|---|
| `expand_threshold` | 0.62 | `VALIDATION_GRAPH_EXPAND` (`config.py:519`) | above the package's 0.55 — moves mass off Full Content onto a richer Description |
| `collapse_floor` | 0.45 | `VALIDATION_GRAPH_COLLAPSE` (`config.py:520`) | package default |
| `link_threshold` | 0.50 | `VALIDATION_GRAPH_LINK` (`config.py:521`) | package default; decides which `similar` edges exist |
| `description_tokens` | 250 tight / 100 large | `VALIDATION_GRAPH_DESCRIPTION_TOKENS` (`config.py:522`) | by window regime: `config.py:TIGHT_WINDOW:319` vs `config.py:LARGE_WINDOW:296`, selected into `config.py:BUDGETS:354` |
| **`body_budget`** | **40000** (was `None`) | `VALIDATION_GRAPH_BODY_BUDGET` (`config.py:523`), `=none` restores the old config | this is what exercises the step down — §5.2 |
| `max_retrieval_cycles` | 4 tight / 8 large | `VALIDATION_GRAPH_MAX_RETRIEVAL_CYCLES` (`config.py:524`) | from the same regime object |
| `reuse_ttl_cycles` | 5 | `VALIDATION_GRAPH_REUSE_TTL` (`config.py:525`) | package default, restated so a sweep can reach it |
| `tags_per_card` | 5 | `VALIDATION_GRAPH_TAGS` (`config.py:526`) | what `find_context` filters on |
| **`neighbors_per_candidate`** | **3** | `VALIDATION_GRAPH_NEIGHBORS` (`config.py:527`), `=0` reproduces every published figure | the `similar` edge's first reader — §6.2 |

These reach the plugin one-for-one in `build_plugins`: `expand_threshold` (`runner.py:395`), `collapse_floor` (`runner.py:396`), `description_tokens` (`runner.py:398`), `body_budget` (`runner.py:399`), `neighbors_per_candidate` (`runner.py:404`).
