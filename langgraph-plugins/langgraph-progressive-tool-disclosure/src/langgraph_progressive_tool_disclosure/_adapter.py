...
def _identified(neutral: NeutralMessage, msg: BaseMessage) -> NeutralMessage:
    ...
    The core's fold copies a message as ``{**message, "content": ...}``, so the key rides through it
    untouched and :func:`to_langchain` can hand the id back to the message it came from.
    """
...
def _blocks_to_content(blocks: list[dict[str, Any]]) -> Any:
    """Render neutral text/json blocks back to a LangChain message ``content`` value.

    A single text block collapses to a plain string — the shape a caller writes by hand — and anything
    richer stays a list of content-part dicts.
    """
...
def to_langchain(message: NeutralMessage) -> list[BaseMessage]:
    ...
    carrier for tool results, and the fold replaces a released tool's ``toolResult`` with a text block on
    that same message. LangChain has no message type holding both, so one neutral message renders as the
    surviving ``ToolMessage`` objects followed by a ``HumanMessage`` with the text. The tool result
    therefore stays immediately behind the tool call, which is the ordering a provider requires, and the
    fold sentence lands behind it.
    ...
    if role == "tool" or results:
        ...
        if body:
            # A fold sentence landing on a tool-result turn. It is per-call content, so it travels as its
            # own message and claims no identity of its own.
            out.append(HumanMessage(content=_blocks_to_content(body), additional_kwargs={ATTACHED_TEXT_KEY: True}))
        return out
...
def to_langchain_list(messages: Sequence[NeutralMessage]) -> list[BaseMessage]:
    """Convert neutral messages back to a flat list of LangChain messages, in order."""
    out: list[BaseMessage] = []
    for message in messages:
        out.extend(to_langchain(message))
    return out
